...
class HansToMatchmakerz extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'hans:matchmakerz';
     ...
     * @var string
     */
    protected $description = 'Transfering Data from Hans to Matchmakerz every Wednesday';
    ...
    public function handle()
    {
        ...
        $query = mysqli_query($connect, "SELECT * FROM profiles WHERE created_at > '2018-07-01' ORDER BY id desc");

        $mmconnect = mysqli_connect('matchmakerz-db.c0egxdbnivt7.ap-south-1.rds.amazonaws.com', 'matchmakerz_db', 'Ja!pur##8107#', 'matchmakerz_db');
        while ($data = mysqli_fetch_array($query)) {
            $id_no = $data['identity_number'];

            $family_sql = "SELECT * FROM families where identity_number='" . $id_no . "' ";
            $family = mysqli_query($connect, $family_sql);
            $fdata = mysqli_fetch_array($family);

            $pref_sql = "SELECT * FROM preferences where identity_number='" . $id_no . "' ";
            $pref = mysqli_query($connect, $pref_sql);
            $pdata = mysqli_fetch_array($pref);

            $photo_sql = "SELECT * FROM photos WHERE identity_number='" . $id_no . "' ";
            $photo = mysqli_query($connect, $photo_sql);
            $photoD = mysqli_fetch_array($photo);
            ...
            if (!empty($photoD['filename'])) {
                if (!empty($fdata['mobile'])) {
                    if (!empty($data['name'])) {
                        ...
                        $weight = $data['weight'];
                        $height = $data['height'];
                        $yearly_income = intval($data['monthly_income'] * 12);

                        if ($data['occupation'] == 'Private Company' || $data['occupation'] == 'Manager' || $data['occupation'] == 'Financial Services/Accounting') {
                            $occupation = 1;
                        } elseif ($data['occupation'] == 'Business/Self Employed' || $data['occupation'] == 'Businessperson') {
                            $occupation = 2;
                        } elseif ($data['occupation'] == 'Civil Services (IAS/ IFS/ IPS/ IRS)' || $data['occupation'] == 'Defence' || $data['occupation'] == 'Government Job' || $data['occupation'] == 'Govt. Services') {
                            $occupation = 3;
                        ...
                        $sub_occupation = $data['sub_occupation'];
                        ...
                        if ($fdata['mother_status'] == 'Passed Away') {
                            $mother_status = 1;
                        } else {
                            $mother_status = 0;
                        }

                        if ($fdata['father_status'] == 'Passed Away') {
                            $father_status = 1;
                        } else {
                            $father_status = 0;
                        }

                        if ($data['food_choice'] == 'Non-Vegetarian') {
                            $food_choice = 1;
                        } else {
                            $food_choice = 0;
                        }

                        if ($data['disability'] == 'Physical Disability') {
                            $disability = 1;
                        } else {
                            $disability = 0;
                        }
                        ...
                        if (!empty($photoD['filename'])) {
                            $profile_pic = "hansmatrimony/uploads/" . $photoD['filename'];
                        } else {
                            $profile_pic = "";
                        }

                        //                        $exist_client = DB::connection('mysql2')->select("SELECT id FROM client_clientprofile WHERE phone_number='$phone_number' LIMIT 1");
                        try {
                            $client_id = DB::connection('mysql2')->insert("INSERT INTO client_clientprofile(name,phone_number,gender,birth_date,current_city,weight,height,profile_photo,yearly_income,occupation,manglik,birth_place,birth_time,education,sub_occupation,family_income,office_address,college,marital_status,caste_id,gotra,mother_status,father_status,food_choice,disability,disabled_part,children,hometown,citizenship,landline,house_type,family_type,is_active,matchmaker_id,married_daughter,married_son,religion,unmarried_daughter,unmarried_son,want_horoscope_match,active_till,whatsapp_number,degree_id,profile_type,id_no,mother_tongue_id) VALUES('$name','$phone_number','$gender','$birth_date','$current_city','$weight','$height','$profile_pic','$yearly_income','$occupation','$manglik','$birth_place','$birth_time','$education','$sub_occupation','$family_income','$office_address','$college','$marital_status','$casteID','$gotra','$mother_status','$father_status','$food_choice','$disability','$disabled_part','$children','$hometown','$citizenship','$landline','$house_type','$family_type',1,3,'$married_daughter','$married_son','$religion','$unmarried_daughter','$unmarried_son','$want_horoscope_match','$active_till','$whatsapp_number','$degreeID','$profile_type','$id_no','1')");
                            $client_id = DB::connection('mysql2')->select("SELECT id FROM client_clientprofile WHERE id_no='$id_no' LIMIT 1");
                            ...
                            if ($pdata['occupation'] == 'Private Company' || $pdata['occupation'] == 'Manager' || $pdata['occupation'] == 'Financial Services/Accounting') {
                                $pref_occupation = 1;
                            } elseif ($pdata['occupation'] == 'Business/Self Employed' || $pdata['occupation'] == 'Businessperson') {
                                $pref_occupation = 2;
                            } elseif ($pdata['occupation'] == 'Civil Services (IAS/ IFS/ IPS/ IRS)' || $pdata['occupation'] == 'Defence' || $pdata['occupation'] == 'Government Job' || $pdata['occupation'] == 'Govt. Services') {
                                $pref_occupation = 3;
                            ...
                            if (!empty($pdata['caste'])) {
                                ...
                                foreach ($pcaste as $caste) {
                                    ...
                                    DB::connection('mysql2')->insert("INSERT INTO client_preferences_caste(preferences_id, caste_id) VALUES('" . $prefID . "','" . $mmcaste['id'] . "')");
                                }
                            } else {
